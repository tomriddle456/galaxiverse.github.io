(function () {
  ...
  let eventCallback = function (e) {
    ...
    if (args.action == "unload") {
      ...
      window.removeEventListener("org.kde.pbi.event", eventCallback, {
        capture: true,
      });
      ...
      let addPlayerToDomEvadingAutoPlayBlocking = function (player) {
        player.registerInDom = () => {
          ...
          if (document.documentElement.contains(player) || player.parentNode) {
      ...
      try {
        ...
        console.warn(
          "Exception executing '" +
            args.mprisCallbackName +
            "' media sessions callback",
          e
        );
      }
        ...
        navigator.share = (data) => {
          return new Promise((resolve, reject) => {
            ...
            if (!window.event || !window.event.isTrusted) {
              return reject(
                new DOMException(
                  "navigator.share can only be called in response to user interaction",
                  "NotAllowedError"
                )
              );
            }
            ...
            if (
              ...
              purposeTransferObject.pendingReject
            ) {
              return reject(
                new DOMException("A share is already in progress", "AbortError")
              );
            }
      ...
      purposeTransferObject.pendingReject(
        new DOMException("Share request aborted", "AbortError")
      );
  ...
  window.addEventListener("org.kde.pbi.event", eventCallback, {
    capture: true,
  });
